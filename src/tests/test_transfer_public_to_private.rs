...
/// Samples the authorization for `transfer_public_to_private`.
pub(crate) fn sample_transfer_public_to_private(rng: &mut TestRng) -> Authorized<CurrentNetwork> {
    // Sample the sender.
    ...
    Credits::transfer_public_to_private(
...
#[test]
fn test_transfer_public_to_private_remote() {
    let rng = &mut TestRng::default();

    // Sample the authorization.
    let authorization = sample_transfer_public_to_private(rng);
...
#[test]
fn test_transfer_public_to_private_local() {
    let rng = &mut TestRng::default();
